...
\textbf{Project Demo:} \url{https://haoyu-gu.github.io/data-mining-demo/} \\
\textbf{Code Repository:} \url{https://github.com/Haoyu-Gu/data-mining}
}
...
\maketitle
\begin{abstract}
Existing symbolic music processing methods predominantly rely on token-based representations, which generate extremely long sequences and struggle to capture global structural patterns. We propose MusiCode, a framework combining representation learning, data mining, and vector quantization for music analysis. Our framework consists of three components: (1) a Transformer-based autoencoder learning compact 768-dimensional embeddings, (2) systematic data mining techniques including UMAP and HDBSCAN clustering, and (3) a theoretical framework for Residual Vector Quantization (RVQ). We implement and validate the first two components on 83,362 music token embeddings. Clustering analysis reveals meaningful patterns with partial voice separation. Baseline comparison demonstrates HDBSCAN achieves 82\% higher Silhouette Score than K-means by identifying density-based structures. Robustness analysis validates stability under 20\% noise, parameter variations, and high reproducibility (ARI $>$ 0.51). Our results validate the effectiveness of combining learned representations with data mining for music analysis.
\end{abstract}
...
In this work, we propose a comprehensive framework that addresses these limitations through the integration of representation learning, data mining techniques, and vector quantization. Our approach, \textbf{MusiCode}, consists of three complementary components designed to enable effective music analysis and compression. In this work, we focus particularly on the first two components---representation learning and data mining---demonstrating their effectiveness through comprehensive experiments on 83,362 music token embeddings. We also present the theoretical framework for the third component, residual vector quantization, providing a foundation for future implementation and integration.
...
\item \textbf{Systematic Data Mining Analysis with Baseline Comparison:} We demonstrate how modern data mining techniques---UMAP dimensionality reduction and HDBSCAN density-based clustering---can be systematically applied to learned music representations to discover latent patterns and identify natural groupings. Through comprehensive parameter search over 360 configurations, our experiments on 83,362 music token embeddings reveal meaningful structure in the embedding space, with partial separation between high and low voice parts. We conduct rigorous baseline comparison with K-means clustering, demonstrating that HDBSCAN achieves 82\% higher Silhouette Score, 43\% lower Davies-Bouldin Index, and 835\% higher Calinski-Harabasz Index, validating its superiority for density-based pattern discovery in learned music representations.
...
\item \textbf{Comprehensive Robustness Analysis:} We conduct extensive robustness validation covering three critical dimensions: (1) noise sensitivity analysis demonstrating stability under up to 20\% Gaussian noise perturbations with graceful degradation, (2) parameter stability analysis showing consistent performance across UMAP dimensionality, neighborhood size, and cluster count variations, and (3) reproducibility assessment confirming high consistency across multiple runs (mean pairwise ARI=0.518, Silhouette CV$<$1.13\%). These results validate the reliability and practical applicability of our approach for real-world music analysis systems.
\end{itemize}
...
The remainder of this paper is organized as follows. Section~\ref{sec:related} reviews related work in symbolic music representation, data mining in music analysis, and vector quantization techniques. Section~\ref{sec:method} describes our representation learning architecture, data mining techniques, and the theoretical framework for residual vector quantization. Section~\ref{sec:experiments} presents our experimental setup, datasets, and implementation details. Section~\ref{sec:results} analyzes our results through visualizations of embedding spaces, clustering quality metrics, and pattern discovery, including baseline comparison with K-means. Section~\ref{sec:robustness} presents comprehensive robustness analysis covering noise sensitivity, parameter stability, and reproducibility. Section~\ref{sec:discussion} discusses the implications of our findings, current limitations, and future directions, followed by our conclusions.
...
The clustering quality metrics demonstrate that HDBSCAN successfully identifies meaningful groupings in the embedding space. To further validate this approach and contextualize its performance, we conduct a comparison with K-means as a classical baseline method in the following subsection.

\subsection{Baseline Comparison with K-Means}

To validate the effectiveness of HDBSCAN and provide a baseline reference, we compare it with the classical K-means algorithm. K-means is widely used for clustering due to its simplicity, deterministic nature, and interpretability. However, it requires pre-specifying the number of clusters and assumes spherical cluster shapes, which may not suit complex musical data distributions.

We configure K-means with $k=512$ clusters to match our target cluster count, while HDBSCAN automatically determines the optimal number of clusters based on density. Table~\ref{tab:kmeans_comparison} presents a comprehensive comparison of both methods.

\begin{table}[t]
\centering
\caption{K-means vs HDBSCAN Clustering Comparison}
\label{tab:kmeans_comparison}
\small
\begin{tabular}{lcc}
\toprule
\textbf{Metric} & \textbf{K-means} & \textbf{HDBSCAN} \\
\midrule
\# Clusters & 512 & 114 \\
Noise Points & 0 (0\%) & 44,660 (53.5\%) \\
\midrule
Silhouette Score & 0.2701 & 0.4914 \\
Davies-Bouldin & 1.2737 & 0.7220 \\
Calinski-Harabasz & 4,822 & 45,118 \\
\midrule
ARI & \multicolumn{2}{c}{0.4684} \\
NMI & \multicolumn{2}{c}{0.8843} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Cluster Assignment Strategy:} K-means assigns all 83,362 points to exactly 512 clusters with no noise points, enforcing complete partitioning of the data. In contrast, HDBSCAN identifies 114 clusters while marking 53.5\% of points as noise, focusing only on high-density core structures.

\textbf{Clustering Quality Comparison:} HDBSCAN demonstrates superior performance across all quality metrics:
\begin{itemize}
\item \textbf{Silhouette Score:} HDBSCAN achieves 0.4914 compared to K-means' 0.2701, an improvement of 82\%. This indicates that HDBSCAN produces more cohesive clusters with better separation.
\item \textbf{Davies-Bouldin Index:} HDBSCAN's score of 0.7220 is 43\% lower than K-means' 1.2737, indicating significantly better cluster separation and distinctness.
\item \textbf{Calinski-Harabasz Index:} HDBSCAN achieves 45,118 compared to K-means' 4,822, an improvement of 835\%. This demonstrates substantially stronger between-cluster variance relative to within-cluster variance.
\end{itemize}

\textbf{Consistency Analysis:} Despite their methodological differences, the two approaches discover partially overlapping structures, as evidenced by moderate Adjusted Rand Index (ARI = 0.4684) and high Normalized Mutual Information (NMI = 0.8843). The moderate ARI suggests that while both methods identify some common groupings, they also capture distinct aspects of the data structure. The high NMI indicates substantial information sharing between the two clusterings.

Figure~\ref{fig:clustering_comparison} visualizes the clustering results of both methods in 2D UMAP space alongside their cluster size distributions and quality metrics.

\begin{figure*}[t]
\centering
\includegraphics[width=0.48\textwidth]{figures/paper_figures/fig4_clustering_results.png}
\includegraphics[width=0.48\textwidth]{figures/paper_figures/fig3_evaluation_metrics.png}
\caption{Comparison of K-means and HDBSCAN clustering results. Left: 2D visualizations showing K-means' 512 uniform clusters versus HDBSCAN's 114 density-based clusters with noise detection. Right: Quality metrics comparison demonstrating HDBSCAN's superior performance across Silhouette Score, Davies-Bouldin Index, and Calinski-Harabasz Index.}
\label{fig:clustering_comparison}
\end{figure*}

\textbf{Interpretation and Trade-offs:} The results reveal fundamental differences in clustering philosophy. K-means' complete assignment strategy ensures all points belong to a cluster, which is useful when every data point must be categorized. However, this comes at the cost of reduced quality metrics, as K-means forces low-density outliers and transitional patterns into clusters where they may not naturally belong.

HDBSCAN's density-based approach identifies only high-confidence clusters, labeling 53.5\% of data as noise. While this results in fewer clusters (114 vs. 512), each cluster represents a genuine high-density core structure. The substantially better quality metrics validate that these clusters are more cohesive and well-separated than K-means' partitions. The noise points likely represent transitional patterns, genuinely unique segments, or low-density regions that do not fit into clear categories---precisely the patterns that reduce K-means' quality scores when forced into clusters.

For music embedding analysis, HDBSCAN's advantages are particularly compelling: automatic cluster number determination avoids arbitrary $k$ selection, arbitrary cluster shape detection accommodates complex learned representations, and noise robustness prevents outliers from degrading cluster quality. These results demonstrate that HDBSCAN is well-suited for discovering meaningful musical patterns in learned embedding spaces, justifying its use as our primary clustering method.
...
To assess the reliability and practical applicability of our clustering approach, we conduct comprehensive robustness analysis covering three critical aspects: noise sensitivity, parameter stability, and reproducibility. While the clustering results in Section 5 demonstrate meaningful patterns, it is crucial to assess whether these findings are robust to data perturbations and parameter variations. These experiments validate that our method produces stable and reliable results under various conditions, ensuring the generalizability of our approach for real-world music analysis applications.

\subsection{Noise Sensitivity}

To evaluate robustness against data perturbations, we add Gaussian noise at various levels (0\%, 1\%, 5\%, 10\%, 20\%) to the embeddings and re-cluster using K-means with $k=512$ clusters. This simulates realistic scenarios where learned representations may be affected by data quality issues, encoding variations, or model uncertainties.

Figure~\ref{fig:noise_sensitivity} shows the noise sensitivity results. When noise level is below 5\%, clustering results remain highly consistent with the baseline (ARI $>$ 0.39, NMI $>$ 0.77), and Silhouette Score decreases only marginally from 0.336 to 0.338. Even at 10\% noise, ARI remains above 0.39 and NMI above 0.77, indicating that major clustering structures are preserved despite substantial perturbations. The 2D visualizations reveal that cluster boundaries become less sharp with increasing noise, but overall structure remains recognizable up to 20\% noise level, where ARI still maintains 0.374 and NMI 0.769.

\begin{figure}[t]
\centering
\includegraphics[width=0.48\textwidth]{figures/robustness_results/robustness_20251119_014340/noise_sensitivity.png}
\caption{Noise sensitivity analysis. Top: ARI and NMI metrics show gradual degradation with increasing noise levels. Bottom: Silhouette Score remains relatively stable, demonstrating robustness. The clustering structure is preserved up to 20\% noise perturbation.}
\label{fig:noise_sensitivity}
\end{figure}

These results demonstrate that our method is robust to moderate noise, which is important for real-world applications where data inevitably contains perturbations from various sources such as quantization errors, model approximations, or preprocessing variations. The gradual rather than abrupt degradation with increasing noise indicates graceful performance characteristics.

\subsection{Parameter Stability}

We test the impact of key hyperparameters on clustering results to assess parameter sensitivity. Specifically, we examine three critical parameters: UMAP dimensionality (n\_components), UMAP neighborhood size (n\_neighbors), and K-means cluster count (n\_clusters).

\textbf{UMAP Dimensionality:} We test n\_components $\in$ \{50, 75, 100\}. Results show minimal variation in Silhouette Score (0.333-0.335) and inertia (12,497-12,539), indicating that the clustering quality remains stable across different intermediate dimensionalities. This suggests our method is not overly sensitive to the specific dimensionality reduction target.

\textbf{UMAP Neighborhood Size:} We test n\_neighbors $\in$ \{10, 15, 30\}. Silhouette Scores vary from 0.330 to 0.342, with n\_neighbors=10 performing slightly better. However, the variation is modest (within 0.012), demonstrating that our results are not critically dependent on this parameter.

\textbf{K-means Cluster Count:} We test n\_clusters $\in$ \{400, 512, 600\}. The Silhouette Score improves from 0.333 (400 clusters) to 0.340 (512 clusters) then to 0.351 (600 clusters). However, inertia decreases as expected when using more clusters (16,239 $\rightarrow$ 12,498 $\rightarrow$ 10,602). The choice of 512 clusters represents a balance between granularity and interpretability, achieving good quality while maintaining a manageable cluster count close to our target.

\begin{figure}[t]
\centering
\includegraphics[width=0.48\textwidth]{figures/robustness_results/robustness_20251119_014340/parameter_stability.png}
\caption{Parameter stability analysis. Left: UMAP n\_components shows minimal impact on clustering quality. Middle: UMAP n\_neighbors demonstrates robust performance across values. Right: K-means n\_clusters shows expected trade-off between granularity and inertia, with 512 achieving good balance.}
\label{fig:parameter_stability}
\end{figure}

Overall, the method demonstrates good stability across most parameter ranges (Figure~\ref{fig:parameter_stability}), with performance varying smoothly rather than exhibiting sharp discontinuities. This indicates that reasonable parameter choices yield consistent results, reducing the need for extensive hyperparameter tuning.

\subsection{Reproducibility}

To verify reproducibility, we perform 10 independent runs of the K-means clustering algorithm and assess consistency across runs. This is particularly important because K-means initialization can affect final cluster assignments.

Figure~\ref{fig:reproducibility} shows multiple runs consistency analysis. For K-means with fixed random\_state=42, all 10 runs produce identical results (perfect reproducibility). When allowing random initialization, pairwise ARI between runs averages 0.518 $\pm$ 0.010, indicating high consistency despite initialization variations. The ARI matrix heatmap shows pairwise ARI values ranging from 0.49 to 0.54, demonstrating that while random initialization introduces some variability, the discovered clustering structures remain highly consistent.

Silhouette Score across 10 runs shows mean of 0.338 with standard deviation of 0.0038, yielding a coefficient of variation below 1.13\%. This very low variability confirms stable clustering quality across runs. The boxplot reveals minimal spread, with all runs achieving Silhouette Scores between 0.332 and 0.347.

\begin{figure}[t]
\centering
\includegraphics[width=0.48\textwidth]{figures/robustness_results/robustness_20251119_014340/multiple_runs_consistency.png}
\caption{Reproducibility analysis across 10 independent runs. Left: ARI matrix heatmap showing high pairwise consistency (0.49-0.54). Right: Silhouette Score distribution with very low variance (mean=0.338, std=0.0038), confirming stable and reproducible clustering quality.}
\label{fig:reproducibility}
\end{figure}

These results confirm that our experiments are highly reproducible, meeting the requirements for rigorous scientific research. The minimal variation across runs indicates that our findings are not artifacts of specific random initializations but reflect genuine structure in the data.

\subsection{Summary of Robustness Findings}

The comprehensive robustness analysis validates three critical properties of our approach:

\begin{itemize}
\item \textbf{Noise Robustness:} Clustering structure remains stable under realistic noise levels (up to 10\% Gaussian noise), with graceful degradation rather than abrupt failure.
\item \textbf{Parameter Stability:} Results are consistent across reasonable hyperparameter ranges, indicating that the method does not require extensive tuning.
\item \textbf{High Reproducibility:} Multiple runs yield highly consistent results (mean pairwise ARI=0.518, Silhouette CV$<$1.13\%), ensuring reliable scientific conclusions.
\end{itemize}

These findings provide strong evidence that our clustering approach is suitable for practical deployment in music analysis systems, where robustness to data variations and parameter choices is essential.
...
Special thanks go to all team members for their dedicated contributions and collaborative spirit. Tingran Meng's meticulous work on data processing ensured the quality and reliability of our dataset. Lekai Qian's innovative model design and implementation brought our theoretical ideas to life. Chenyuan Hong's excellent presentation skills effectively communicated our findings. The collaborative environment and support within our team made this project both productive and enjoyable.
...
\label{tab:division}
\begin{tabularx}{\textwidth}{l>{\raggedright\arraybackslash}Xp{4.5cm}}
\toprule
\textbf{Team Member} & \textbf{Responsibilities and Contributions} & \textbf{Main Role} \\
\midrule
Lekai Qian & Data collection and preprocessing pipeline design; MIDI tokenization and embedding extraction; Transformer autoencoder architecture design and implementation; Model training and optimization & Data Processing \& Method Design \\
\midrule
Tingran Meng & UMAP-HDBSCAN parameter search implementation (360 configurations); K-means baseline comparison experiments; Clustering quality metrics evaluation; Comparative analysis between clustering methods & Data Mining \& Multi-Method Comparison \\
\midrule
Chenyuan Hong & Presentation slide design and preparation; Oral presentation delivery; Comprehensive visualization dashboard creation; Figure design and aesthetic refinement; Robustness analysis experiment execution and monitoring & Presentation \& Visualization \\
\midrule
Haoyu Gu & Literature review and related work analysis; Experimental result analysis and interpretation; Statistical analysis of clustering patterns and quality metrics; Report writing and documentation & Report Writing \& Data Analysis \\
\bottomrule
\end{tabularx}
\end{table*}
...
This section provides supplementary technical details about our experimental setup and implementation choices.

\subsubsection{Transformer Autoencoder Configuration}

Our Transformer autoencoder was trained with the following detailed configuration:
\begin{itemize}
\item \textbf{Training Data:} 83,362 music segments extracted from diverse MIDI files, tokenized using REMI representation with vocabulary size of 2,048 tokens
\item \textbf{Training Procedure:} 100 epochs with batch size 64, total training time approximately 48 hours on NVIDIA A100 GPU
\item \textbf{Optimization:} Adam optimizer with initial learning rate $1 \times 10^{-4}$, cosine annealing learning rate schedule with 4,000 warmup steps
\item \textbf{Regularization:} Dropout rate 0.1 applied to all attention and feed-forward layers, label smoothing 0.1 for reconstruction loss
\item \textbf{Early Stopping:} Validation loss monitored with patience of 10 epochs, best model selected based on reconstruction accuracy
\end{itemize}

\subsubsection{UMAP-HDBSCAN Parameter Search Details}

Our systematic parameter search explored the following configuration space:
\begin{itemize}
\item \textbf{UMAP Parameters:} n\_components $\in$ \{50, 75, 100\}, n\_neighbors $\in$ \{15, 30\}, min\_dist $\in$ \{0.0, 0.1\}, metric='cosine'
\item \textbf{HDBSCAN Parameters:} min\_cluster\_size $\in$ \{30, 50, 80, 100, 120, 150\}, min\_samples $\in$ \{5, 10, 15, 20\}, cluster\_selection\_epsilon $\in$ \{0.0, 0.1, 0.2\}
\item \textbf{Search Strategy:} Grid search over all combinations, total 360 configurations evaluated
\item \textbf{Selection Criterion:} Configuration producing cluster count closest to 512 while maintaining Silhouette Score $>$ 0.50
\item \textbf{Computational Cost:} Total search time approximately 12 hours on Intel Xeon CPU with 64 cores
\end{itemize}

\subsubsection{Baseline K-means Implementation}

For fair comparison with HDBSCAN, we implemented K-means with the following settings:
\begin{itemize}
\item \textbf{Algorithm:} K-means++ initialization for better convergence
\item \textbf{Cluster Count:} $k = 512$ to match target granularity
\item \textbf{Convergence:} Maximum 300 iterations with tolerance $1 \times 10^{-4}$
\item \textbf{Multiple Runs:} 10 independent runs with different random seeds for reproducibility assessment
\item \textbf{Input Data:} Same UMAP-reduced 75-dimensional embeddings used for HDBSCAN clustering
\end{itemize}

\subsubsection{Robustness Testing Procedures}

Detailed procedures for robustness experiments:
\begin{itemize}
\item \textbf{Noise Addition:} Gaussian noise with standard deviation proportional to embedding norm (0\%, 1\%, 5\%, 10\%, 20\% of mean norm)
\item \textbf{Parameter Variation:} Systematic testing of each parameter while holding others constant at optimal values
\item \textbf{Reproducibility Assessment:} 10 independent runs with random seeds \{42, 43, ..., 51\}, pairwise ARI computed for all pairs
\item \textbf{Metrics Computation:} All clustering quality metrics computed using scikit-learn v1.3 implementations with default settings
\end{itemize}

\subsection{AI Usage Report}

In accordance with academic transparency standards, we report our use of AI tools during this project. \textbf{AI technologies were employed solely as auxiliary tools to assist with code editing and report composition. All core intellectual contributions, experimental design, algorithmic implementation, and scientific analysis were conducted by team members.}

\subsubsection{Scope of AI Assistance}

AI tools (including ChatGPT, Claude, and GitHub Copilot) provided limited assistance in the following areas:

\textbf{Code Editing Support:}
\begin{itemize}
\item Syntax suggestions for standard Python library functions (NumPy, scikit-learn)
\item Debugging assistance for identifying common programming errors (e.g., dimension mismatches, index errors)
\item Code formatting recommendations following PEP 8 style guidelines
\end{itemize}

\textbf{Report Writing Assistance:}
\begin{itemize}
\item Grammar and spelling corrections for English academic writing
\item Sentence restructuring suggestions to improve clarity
\item LaTeX formatting assistance (e.g., table syntax, figure placement)
\item Translation assistance for technical terminology
\end{itemize}

\subsubsection{Human-Led Core Contributions}

\textbf{All substantive research activities were conducted entirely by team members:}

\begin{itemize}
\item \textbf{Research Design:} Problem formulation, methodology selection (UMAP, HDBSCAN, RVQ framework), and experimental protocol design were conceptualized and finalized by the research team through collaborative discussions.

\item \textbf{Algorithm Implementation:} The Transformer autoencoder architecture, UMAP-HDBSCAN pipeline, baseline comparison experiments, and robustness analysis were implemented from scratch by team members. All hyperparameter tuning, parameter search strategies, and optimization decisions were made by the team based on experimental results and domain knowledge.

\item \textbf{Experimental Execution:} Data collection, preprocessing, model training (100 epochs, 48 hours), parameter search (360 configurations, 12 hours), clustering experiments, and robustness testing were entirely executed and monitored by team members.

\item \textbf{Scientific Analysis:} Interpretation of clustering results, quality metric analysis, comparison between K-means and HDBSCAN, robustness assessment, and drawing scientific conclusions were performed independently by team members through careful examination of experimental outputs.

\item \textbf{Technical Writing:} The theoretical framework for RVQ (Section 3.6), methodology descriptions, experimental design rationale, result interpretation, discussion of findings, and all technical content were authored by team members. AI tools only assisted with linguistic refinement after content creation.
\end{itemize}

\subsubsection{Verification and Validation}

All AI-generated suggestions underwent rigorous human review:
\begin{itemize}
\item \textbf{Code Verification:} Every code snippet suggested by AI tools was manually reviewed, tested with unit tests, and validated against expected outputs before integration.
\item \textbf{Content Validation:} All technical descriptions, mathematical formulations, and experimental interpretations were verified by team members against original literature and experimental data.
\item \textbf{Quality Control:} Team members cross-checked all sections of the report to ensure accuracy, consistency, and alignment with research objectives.
\end{itemize}
...
We affirm that:
\begin{itemize}
\item \textbf{Intellectual Ownership:} All research questions, hypotheses, methodological choices, experimental designs, and scientific conclusions originated from team members' independent thinking and collaborative discussions.
\item \textbf{Technical Expertise:} The implementation of algorithms, interpretation of results, and analysis of findings reflect team members' understanding of data mining, machine learning, and music information retrieval.
\item \textbf{Original Contribution:} The integration of representation learning, HDBSCAN clustering, baseline comparison, and comprehensive robustness analysis represents original work conceived and executed by the research team.
\item \textbf{AI as Tool, Not Collaborator:} AI tools functioned purely as productivity aids (similar to spell-checkers or IDEs), not as intellectual collaborators or co-authors.
\end{itemize}

This transparent disclosure ensures proper attribution of intellectual contributions while acknowledging the auxiliary role of modern AI tools in academic research workflows.

\subsection{Supplementary Visualizations}

This section presents additional visualizations that were generated during our analysis but not included in the main text. These figures provide complementary perspectives on the data mining process and clustering results.

\subsubsection{Alternative Clustering Visualization}

Figure~\ref{fig:tab20_clustering} presents an alternative visualization of our clustering results using the Tab20 colormap, which provides distinct categorical colors for better visual differentiation of individual clusters.

\begin{figure}[h]
\centering
\includegraphics[width=0.48\textwidth]{figures/3_clustering_scatter_tab20.png}
\caption{HDBSCAN clustering results visualized with Tab20 colormap. This categorical color scheme helps distinguish individual clusters more clearly than continuous colormaps, particularly useful for identifying specific cluster boundaries in dense regions.}
\label{fig:tab20_clustering}
\end{figure}

\subsubsection{Parameter Search Analysis}

Figures~\ref{fig:cluster_dist}~through~\ref{fig:best_combos} present comprehensive analysis of our systematic parameter search process for UMAP-HDBSCAN clustering, which involved testing 360 different parameter combinations to achieve our target of approximately 512 clusters.

\begin{figure}[h]
\centering
\includegraphics[width=0.48\textwidth]{figures_appendix/1_cluster_count_distribution.png}
\caption{Distribution of cluster counts across all 360 parameter combinations tested. The red dashed line indicates our target of 512 clusters, while the orange dashed line shows the mean of 179 clusters across all configurations. Most configurations produced 100-300 clusters, requiring careful parameter tuning to achieve higher cluster counts.}
\label{fig:cluster_dist}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.48\textwidth]{figures_appendix/2_clusters_by_umap_dimension.png}
\caption{Cluster counts grouped by UMAP target dimension (50, 75, 100). This analysis shows how the intermediate dimensionality after UMAP reduction affects the number of clusters discovered by HDBSCAN, helping identify optimal dimensionality settings.}
\label{fig:clusters_umap_dim}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.48\textwidth]{figures_appendix/3_clusters_vs_min_cluster_size.png}
\caption{Relationship between HDBSCAN's min\_cluster\_size parameter and resulting cluster count. As expected, larger minimum cluster sizes produce fewer clusters by requiring more points to form valid clusters.}
\label{fig:clusters_vs_minsize}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.48\textwidth]{figures_appendix/4_noise_ratio_distribution.png}
\caption{Distribution of noise ratios across different parameter combinations. The noise ratio indicates the proportion of points HDBSCAN classifies as not belonging to any dense cluster, reflecting the algorithm's robustness to outliers.}
\label{fig:noise_dist}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.48\textwidth]{figures_appendix/5_silhouette_vs_clusters.png}
\caption{Silhouette Score plotted against number of clusters, colored by noise ratio. This scatter plot reveals the quality-quantity tradeoff: configurations producing more clusters generally show moderate silhouette scores (0.45-0.50), while lower cluster counts sometimes achieve higher scores but miss fine-grained patterns.}
\label{fig:silhouette_vs_clusters}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.48\textwidth]{figures_appendix/6_heatmap_min_samples_vs_min_cluster_size.png}
\caption{Heatmap showing the interaction between min\_samples and min\_cluster\_size parameters. Darker regions indicate parameter combinations that produce cluster counts closer to our target of 512.}
\label{fig:heatmap_samples_size}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.48\textwidth]{figures_appendix/7_heatmap_umap_vs_min_cluster_size.png}
\caption{Heatmap displaying the relationship between UMAP n\_components and HDBSCAN min\_cluster\_size. This visualization helps identify optimal parameter regions where both UMAP dimensionality and cluster size constraints work synergistically.}
\label{fig:heatmap_umap_size}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.48\textwidth]{figures_appendix/8_clusters_vs_noise_ratio.png}
\caption{Cluster count plotted against noise ratio, revealing the tradeoff between cluster granularity and noise tolerance. Higher cluster counts are typically associated with moderate noise ratios (40-50\%), indicating that fine-grained clustering naturally identifies more outlier points.}
\label{fig:clusters_vs_noise}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.48\textwidth]{figures_appendix/9_top10_best_combinations.png}
\caption{Top 10 parameter combinations that produced cluster counts closest to our target of 512. The best configurations achieved 295 clusters (about 58\% of target), demonstrating the challenge of reaching very high cluster counts while maintaining quality. Parameters are displayed as n\_comp (UMAP components), min\_cls (min\_cluster\_size), and min\_samp (min\_samples).}
\label{fig:best_combos}
\end{figure}

% \begin{figure}[h]
% \centering
% \includegraphics[width=0.95\textwidth]{figures_appendix/10_comprehensive_dashboard.png}
% \caption{Comprehensive dashboard from parameter search showing the overall distribution and quality metrics. This integrated view helped guide our final parameter selection by simultaneously displaying cluster counts, noise ratios, and quality scores.}
% \label{fig:appendix_dashboard}
% \end{figure}

These supplementary visualizations provide deeper insights into the parameter search process and alternative representation strategies explored during our study. They demonstrate the systematic approach taken to optimize clustering quality while targeting specific cluster granularity.
